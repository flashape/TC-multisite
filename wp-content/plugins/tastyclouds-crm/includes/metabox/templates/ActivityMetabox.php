...
function onActivityMetaboxFooterAction() {
	...
	function loadMail(){
		...
		var data = {};
		data.action = 'panalo_get_mail_for_contact';
	...
	function saveActivity(row){
		...
		var data = {};
		data.action = newModel.id == 0 ? 'panalo_insert_activity' : 'panalo_update_activity'
		
		data.model = stringify(newModel);
	...
	function deleteActivity(row) {
		var data = {};
		data.action = 'panalo_delete_activity';
		
		data.model = stringify(row.data("model"));
		data.postID = jQuery('#post_ID').val();
		row.remove();
	...
	function getActivities() {
		var data = {};
		data.action = 'panalo_get_activities_for_post';
