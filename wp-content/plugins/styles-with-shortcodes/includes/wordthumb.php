...
define ('MEMORY_LIMIT', '99M');				// Set PHP memory limit
...
define ('FILE_CACHE_DIRECTORY', '');			// Directory where images are cached. Left blank it will use the system temporary directory (recommended for security)
...
class wordthumb {
	...
	public function __construct(){
		...
		$this->cacheDirectory = FILE_CACHE_DIRECTORY ? FILE_CACHE_DIRECTORY : sys_get_temp_dir(); 
		...
		$this->cachefile = $this->cacheDirectory . '/' . $cachePrefix . md5($_SERVER ['QUERY_STRING'] . VERSION) . FILE_CACHE_SUFFIX;
	...
	protected function tryServerCache(){
		...
		if(file_exists($this->cachefile)){
			...
			if($this->isURL){
				$this->debug(3, "Checking cachefile for external request");
				if(filesize($this->cachefile) < 1){
					...
					if(time() - filemtime($this->cachefile) > WAIT_BETWEEN_FETCH_ERRORS){
						...
						$this->error("An error occured fetching image from " . $this->src);
						return false; 
					}
				}
			} else {
	...
	protected function serveInternalImage(){
		...
		if(! $localImage){
			$this->error("Could not find the image {$this->src}");
			return false;
		}
		if(! is_file($localImage)){
			$this->error("The file {$this->src} is not a valid file.");
			return false;
		}
		$fileSize = filesize($localImage);
		if($fileSize > MAX_FILE_SIZE){
			$this->error("The file {$this->src} is greater than the maximum allowed file size.");
			return false;
		}
		if($fileSize <= 0){
			$this->error("The file {$this->src} is <= 0 bytes.");
			return false;
		}
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if ($image === false) {
			return $this->error('Unable to open image : ' . $localImage);
		}
		...
		$fh = fopen($lockFile, 'w');
		if(! $fh){
			return $this->error("Could not open file $lockFile for writing.");
		}
		if(flock($fh, LOCK_EX)){
			if(preg_match('/^image\/(?:jpg|jpeg)$/i', $mimeType)){ 
				imagejpeg($canvas, $this->cachefile, $quality); 
			} else if(preg_match('/^image\/(?:png|gif)$/i', $mimeType)){ 
			...
			fclose($fh);
			unlink($lockFile);
			return $this->error("Could not get a lock on file {$this->cachefile} for writing.");
		}
		imagedestroy($canvas);
		$this->cleanCache();
		return true;
	...
	protected function serveWebshot(){
		...
		$tempfile = tempnam(sys_get_temp_dir(), 'wordthumb_webshot');
		$url = $this->src;
	...
	protected function serveExternalImage(){
		...
		$tempfile = tempnam(sys_get_temp_dir(), 'wordthumb');
		...
		if(function_exists('curl_init')){
			...
			if(! $curlResult){
				...
				$this->error("Error reading {$this->src} from remote host: " . curl_error($curl));
				return false;
			}
		} else {
			...
			if(! file_put_contents($tempfile, $img)){
				$this->error("Could not write the contents of remote file {$this->src} to temporary file $tempfile.");
				return false;
			}
		}
		...
		if(! preg_match("/^image\/(?:jpg|jpeg|gif|png)$/i", $imageInfo['mime'])){
			...
			unlink($this->cachefile);
			touch($this->cachefile);
			$this->error("The remote file {$this->src} is not a valid image.");
			return false;
		}
		if($this->processImageAndWriteToCache($tempfile)){
			$this->debug(3, "Image processed succesfully. Serving from cache");
	...
	protected function serveCacheFile(){
		...
		$imgInfo = getimagesize($this->cachefile);
		if(! preg_match('/^image\//', $imgInfo['mime'])){
			$this->error("Invalid mime type for image. This image can't be served.");
			return false;
		}
		...
		if (@readfile ($this->cachefile)) {
			$this->debug(3, "Served using readfile straight to STDOUT");
			return true;
		} else {
