...
function createSwiftmailTransport(){
	$transport = Swift_SmtpTransport::newInstance('smtp.gmail.com', 465, 'ssl')
	  ->setUsername('smpttest@f1fd.com')
	  ->setPassword('m1ll10n$')
	  ;
...
function tc_send_customer_order_email($data){
	...
	$title = "Your order from Tasty Clouds Cotton Candy Company Store (#{$orderID})";
	...
	//Create the Transport
	$transport = createSwiftmailTransport();
...
function tc_send_admin_order_email($data){
	//$user = new WP_user($data['userID']);
	//$postID = $data['postID'];
	...
	$title = "New Order for Tasty Clouds Cotton CAndy Company Store (#{$orderID})";
...
function tc_send_order_updated_email($data){
